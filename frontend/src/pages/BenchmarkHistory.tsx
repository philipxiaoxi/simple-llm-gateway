...
function RunDetails({ runId }: { runId: number }) {
  ...
  return (
    <div className="border-t border-line bg-black/10 px-4 py-3">
      <div className="mb-2 text-xs text-mist">提示词：{data?.prompt}</div>
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden p-0">
        ...
        <div className="grid gap-3 p-3 md:hidden">{runs.map((run) => <div key={run.id} className="border border-line"><button type="button" className="w-full p-3 text-left" onClick={() => setExpanded(expanded === run.id ? null : run.id)}><div className="flex items-center justify-between"><span className="font-medium">{formatTime(run.created_at)}</span>{expanded === run.id ? <ChevronDown size={16} /> : <ChevronRight size={16} />}</div><div className="mt-2 truncate text-sm">{run.prompt}</div><div className="mt-1 text-xs text-mist">{run.success_count} / {run.result_count} 成功</div></button>{expanded === run.id ? <RunDetails runId={run.id} /> : null}</div>)}</div>
        {expanded && runs.some((run) => run.id === expanded) ? <div className="hidden md:block"><RunDetails runId={expanded} /></div> : null}
      </Card>
